...
$sql = "SELECT trx_id, name, email, twithdraw, created_at FROM transactions WHERE email = ?";

if ($search) {
    // If there's a search term, adjust the query to search in trx_id, name, or email
    $sql = "SELECT trx_id, name, email, twithdraw, created_at 
            FROM transactions 
            WHERE email = ? 
...
if ($search) {
    ...
    $stmt->bind_param("ssss", $logged_in_email, $like_pattern, $like_pattern, $like_pattern); // Binding for multiple LIKE searches
} else {
    $stmt->bind_param("s", $logged_in_email); // Binding when there's no search term
}
...
<body>
    ...
    <div class="body-wrapper">
        <div class="d-flex mb-30 flex-wrap gap-3 justify-content-between align-items-center">
            <h6 class="page-title">Withdraw History</h6>
