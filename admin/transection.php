...
$sql = "SELECT id, trx_id, name, email, amount, tdeposit, available_earnings, twithdraw, created_at FROM transactions";

// If there's a search term, adjust the query to include a WHERE clause
if ($search) {
    $sql = "SELECT id, trx_id, name, email, amount, tdeposit, available_earnings, twithdraw, created_at 
            FROM transactions 
...
<head>
    ...
    <title>Transactions History | Investify</title>
...
<body>
    ...
    <div class="body-wrapper">
        <div class="d-flex mb-30 flex-wrap gap-3 justify-content-between align-items-center">
            <h6 class="page-title">Transactions History</h6>
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    $wedit_url = 'wedit.php?id=' . urlencode($row['id']); // URL for editing
                                    $earndelete_url = 'earn-delete.php?id=' . urlencode($row['id']); // URL for editing
                                    

                                    echo '<tr class="text-center">';
                                    echo '<td>' . htmlspecialchars($row['trx_id']) . '</td>'; 
                                    echo '<td>' . htmlspecialchars($row['name']) . '</td>'; 
                                    echo '<td>' . htmlspecialchars($row['email']) . '</td>'; 
                                    echo '<td >' . htmlspecialchars($row['amount']) . ' USD</td>';
                                    echo '<td class="budget text-center">' . htmlspecialchars($row['tdeposit']) .' '. 'USD' .'|'.'   <a href="' . htmlspecialchars($edit_url) . '"><i class="fa-regular fa-pen-to-square"></i></a></td>'; 
                                    
                                    
                                    // Assuming the PHP loop from your table generation code
                                    echo '<td class="budget text-center">' . htmlspecialchars($row['available_earnings']) .' '. 'USD' .'</i></a></td>'; 

                    
                                    echo '<td class="budget text-center">' . htmlspecialchars($row['twithdraw']) .' '. 'USD' .'|'.'   <a href="' . htmlspecialchars($wedit_url) . '"><i class="fa-regular fa-pen-to-square"></i></a></td>'; 
                                    echo '<td>' . htmlspecialchars($row['created_at']) . '</td>'; 
                                                                    
                                    // Fetch and display status data from investments table
                                    $sql_status = "SELECT status FROM investments WHERE trx_id = ?";
                                    $stmt_status = $conn->prepare($sql_status);
                                    $stmt_status->bind_param("s", $row['trx_id']);
                                    $stmt_status->execute();
                                    $result_status = $stmt_status->get_result();

                                    if ($result_status->num_rows > 0) {
                                        $row_status = $result_status->fetch_assoc();
                                        echo '<td>' . htmlspecialchars($row_status['status']) . '</td>';
                                    } else {
                                        echo '<td>active</td>';
                                    }
